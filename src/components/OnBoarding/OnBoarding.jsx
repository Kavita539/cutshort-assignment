...
<div className="tabHeader">
  <span className="tabHeader-main">{tabHeadings[tabNumber-1].main}</span>
  <span className="tabHeader-sub">{tabHeadings[tabNumber-1].sub}</span>
</div>
}

    {tabNumber === 1 &&
    <form onSubmit={formSubmitHandler} style={{width: '100%'}}>
        <Input id="fullName" label="Full Name" placeholder='Steve Jobs' value={user.fullName} onChange={(value)=>
        setUser({...user, 'fullName': value })}
        minLength="2"
        maxLength="25"/>
        <Input id="displayName" label="Display Name" placeholder='Steve' value={user.displayName} onChange={(value)=>
    ...
    {tabNumber === 2 &&
    <form onSubmit={formSubmitHandler} style={{width: '100%'}}>
        <Input id="workspaceName" label="Workspace Name" placeholder='Eden' value={workSpace.workspaceName}
            onChange={(value)=> setWorkSpace({ ...workSpace, 'workspaceName': value })}
        minLength="2"
        maxLength="25"/>
        <Input id="workSpaceURL" label="Workspace URL" placeholder='Example' value={workSpace.workspaceURL}
    ...
    <div className="cards-container">
      {cardContent.map((card) => {
        return <TextCard key={card.id} isActive={card.id === usage.usage} card={card} setUsage={setUsage} /> //handleCardSelect={handleCardSelect}
      })}
    </div>
    <Button text="Create Workspace" clickHandler={clickHandler} />
  </div>}
